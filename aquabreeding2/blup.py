...
def nrm_cpp(par_inf, pro_inf):
    ...
    aq.nrm(n_mat, n_founder, n_par + n_progeny, dict_s, dict_d)
...
def convert_gmatrix(pro_inf, gen_array, which_mat):
    '''
    ...
    Args:
        ...
        which_mat (int): 0: normal G matrix, 1: G matrix for GP
...
def bv_estimation2(phe_inf, g_mat, x_i, y_vec):
    ...
    Args:
        ...
        y_vec (numpy.ndarray): Phenotypes of training population
    Note:
        Fixed effect is not implement yet.
    '''
    def restricted_ml(lam, n_p, theta_vec, omega_sq):
    ...
    x_mat = np.ones((n_sample, 1), dtype=np.float64)
    u_1, s_1, vh_1 = np.linalg.svd(x_mat)
    r_i = np.max(np.where(s_1 > 1e-8)) + 1
    x_mat = u_1[:, 0: r_i].reshape(n_sample, r_i)
    z_mat = np.identity(n_all, dtype=np.float64)
    z_mat = z_mat[0: n_sample, :]
    # the number of fixed effects
    n_p = x_mat.shape[1]
    # Convert rrBLUP/R into python
    xt_mat = x_mat.T
